...
public class ScheduleController {
    ...
    public ResponseEntity<Response<List<PatientScheduleResponse>>> getSchedulesByDoctor(
        ...
        try {
            ...
            List<Schedule> schedules = scheduleService.getAppointmentsByDoctorId(doctorId, startOfDay, endOfDay,
    ...
    public ResponseEntity<Response<List<DoctorScheduleReponse>>> getSchedulesByDoctor(
        ...
        UUID effectiveDoctorId = doctorId != null ? doctorId : currentUserUtils.getCurrentUser().getId();
        List<Schedule> schedules = scheduleService.getSchedulesByDoctorId(effectiveDoctorId, status, from, to);
    ...
    public ResponseEntity<Response<List<DoctorScheduleReponse>>> getSchedulesByPatient(
            ...
            @RequestParam(defaultValue = "PENDING") List<Schedule.Status> status) {
        List<Schedule> schedules = scheduleService.getSchedulesByPatientId(currentUserUtils.getCurrentUser().getId(),
                status, from, to);
    ...
    public ResponseEntity<Response<ScheduleResponse>> getScheduleResultById(@PathVariable UUID scheduleId) {
        ...
        if (schedule.getDoctor().getId() != currentUserUtils.getCurrentUser().getId()
                || schedule.getPatient().getId() != currentUserUtils.getCurrentUser().getId()) {
    ...
    @PostMapping("/result")
    public ResponseEntity<Response<DoctorScheduleReponse>> addScheduleResult(
            @RequestBody ScheduleResultRequest request) {
    ...
    // public ResponseEntity<Response<ScheduleResponse>> addSchedule(@RequestBody
    // AddScheduleToPhaseRequest request) {
    // Schedule schedule = scheduleService.addScheduleToPhase(request,
    ...
    public ResponseEntity<Response<DoctorScheduleReponse>> getSpecitficScheduleForPatient(
        ...
        Schedule schedule = scheduleService.getScheduleById(scheduleId);
        if (!schedule.getPatient().getId().equals(currentUserUtils.getCurrentUserId()))
            throw new UnauthorizedAccessException("The patient do not have access to this schedule");

        return ResponseEntity.ok(
                new Response<>(scheduleMapper.toDoctorScheduleRespone(schedule), "Schedule retreive successfully."));
    }
    ...
    public ResponseEntity<Response<DoctorScheduleReponse>> getSpecitficScheduleForDoctor(
            @PathVariable UUID scheduleId) {
        Schedule schedule = scheduleService.getScheduleById(scheduleId);
        if (!schedule.getDoctor().getId().equals(currentUserUtils.getCurrentUserId()))
            throw new UnauthorizedAccessException("The patient do not have access to this schedule");
    ...
    public ResponseEntity<Response<List<PatientScheduleResponse>>> getTodayPatientSchedules() {
        List<Schedule> schedules = scheduleService
                .getTodayScheduleForPatient(currentUserUtils.getCurrentUser().getId());
        List<PatientScheduleResponse> responses = schedules.stream()
